...
class Transposition: public Encryptor{
	public:
		Transposition(const std::string& numericKey){
...
std::string Transposition::encrypt(const std::string& input){
	...
	int colWidth = numericKey.length();
	int rows = inputLen/colWidth + 1;
	std::string cipher = "";
	char table[rows][colWidth];
	int k=0;
	...
	for(int i=0; i < rows; i++){
		for(int j=0; j < numericKey.length(); j++){
			int col = numericKey[j] - '1';
			cipher += table[i][col];
		}
	}
...
void encryption(const std::string& input, std::string numericKey){
	// create an object of class substitution
...
std::string removeSpacesAndToUpper(const std::string s){
	std::string tmp = "";
...
int main(){
	...
	while(true){
		...
		// remove spaces and convert to all upper case letters
		input = removeSpacesAndToUpper(input);
